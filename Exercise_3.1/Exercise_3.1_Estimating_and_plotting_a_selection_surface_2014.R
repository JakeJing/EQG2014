...
new.body=(mean(BODY)-BODY)/sd(BODY)
new.tail=(mean(TAIL)-TAIL)/sd(TAIL)
...
#we should have from the results above, the following values for selection gradients
beta1= -0.031408  
beta2= -0.001504
gamma11 = -0.010578 
gamma22 = -0.005914 
...
#The eigenvectors of omega should be the same as those of the gamma-matrix, but in reverse order
#Let�s check
eigen(omega)
eigen(gamma)
